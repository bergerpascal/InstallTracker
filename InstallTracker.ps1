...
function Invoke-Snapshot {
  ...
  try {
    ...
    if ($SelectedMode -eq 'Post') {
      ...
      function ConvertTo-HtmlEscape([string]$s) {
        if ($null -eq $s) { return '' }
        $s -replace '&','&amp;' -replace '<','&lt;' -replace '>','&gt;' -replace '"','&quot;'
      }

      # Helper: render an array of objects as an HTML table with given columns
      function ConvertTo-HtmlTable {
        ...
        foreach ($row in $Rows) {
          ...
          foreach ($col in $Columns) {
            ...
            [void]$sb.Append("<td>$(ConvertTo-HtmlEscape $val)</td>")
          }
          [void]$sb.Append('</tr>')
        }
      ...
      function New-HtmlSection {
        param([string]$Id, [string]$Title, [int]$Count, [string]$BadgeClass, [string]$Content)
        $open = if ($Count -gt 0) { ' open' } else { '' }
        return @"
<details id="sec-$Id"$open>
  <summary>
...
"@
      ...
      $sectionsHtml = ''
      $sectionsHtml += New-HtmlSection -Id 'svc-add' -Title 'New Services' -Count $svcDiff.Added.Count -BadgeClass 'add' `
        -Content (ConvertTo-HtmlTable -Rows ($svcDiff.Added | Sort-Object Name) -Columns @('Name','DisplayName','StartMode','State','PathName') -RowClass 'add')
      $sectionsHtml += New-HtmlSection -Id 'svc-rem' -Title 'Removed Services' -Count $svcDiff.Removed.Count -BadgeClass 'rem' `
        -Content (ConvertTo-HtmlTable -Rows ($svcDiff.Removed | Sort-Object Name) -Columns @('Name','DisplayName','StartMode','State','PathName') -RowClass 'rem')

      $sectionsHtml += New-HtmlSection -Id 'tsk-add' -Title 'New Scheduled Tasks' -Count $tskDiff.Added.Count -BadgeClass 'add' `
        -Content (ConvertTo-HtmlTable -Rows ($tskDiff.Added | Sort-Object TaskPath,TaskName) -Columns @('TaskPath','TaskName','State','Actions') -RowClass 'add')
      $sectionsHtml += New-HtmlSection -Id 'tsk-rem' -Title 'Removed Scheduled Tasks' -Count $tskDiff.Removed.Count -BadgeClass 'rem' `
        -Content (ConvertTo-HtmlTable -Rows ($tskDiff.Removed | Sort-Object TaskPath,TaskName) -Columns @('TaskPath','TaskName','State','Actions') -RowClass 'rem')

      $sectionsHtml += New-HtmlSection -Id 'run-add' -Title 'New Run/RunOnce Entries' -Count $runDiff.Added.Count -BadgeClass 'add' `
        -Content (ConvertTo-HtmlTable -Rows ($runDiff.Added | Sort-Object HivePath,Name) -Columns @('HivePath','Name','Value') -RowClass 'add')
      $sectionsHtml += New-HtmlSection -Id 'run-rem' -Title 'Removed Run/RunOnce Entries' -Count $runDiff.Removed.Count -BadgeClass 'rem' `
        -Content (ConvertTo-HtmlTable -Rows ($runDiff.Removed | Sort-Object HivePath,Name) -Columns @('HivePath','Name','Value') -RowClass 'rem')

      $sectionsHtml += New-HtmlSection -Id 'uni-add' -Title 'New Installed Apps' -Count $uninstallDiff.Added.Count -BadgeClass 'add' `
        -Content (ConvertTo-HtmlTable -Rows ($uninstallDiff.Added | Sort-Object DisplayName) -Columns @('DisplayName','DisplayVersion','HivePath','SubkeyName') -RowClass 'add')
      $sectionsHtml += New-HtmlSection -Id 'uni-rem' -Title 'Removed Installed Apps' -Count $uninstallDiff.Removed.Count -BadgeClass 'rem' `
        -Content (ConvertTo-HtmlTable -Rows ($uninstallDiff.Removed | Sort-Object DisplayName) -Columns @('DisplayName','DisplayVersion','HivePath','SubkeyName') -RowClass 'rem')

      $sectionsHtml += New-HtmlSection -Id 'fld-add' -Title 'New Folders' -Count $fldDiff.Added.Count -BadgeClass 'add' `
        -Content (ConvertTo-HtmlTable -Rows ($fldDiff.Added | Sort-Object FullName) -Columns @('FullName','CreationTime') -RowClass 'add')

      $sectionsHtml += New-HtmlSection -Id 'lnk-add' -Title 'New Shortcuts' -Count $lnkDiff.Added.Count -BadgeClass 'add' `
...
<body>
<header>
  <h1>InstallTracker &mdash; Change Report</h1>
...
"@
      ...
      $snapshotRootDir = Join-Path $scriptDir $SnapshotRoot
      $txtPath = Join-Path $snapshotRootDir ("ChangeReport_{0}.html" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
      [System.IO.File]::WriteAllText($txtPath, $htmlDoc, [System.Text.Encoding]::UTF8)
